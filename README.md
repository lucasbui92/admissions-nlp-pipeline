...
```bash
python run.py --mode <mode> --output_name <name> [--input <path>] [--metric <metric>] [--include_matches] [--stage <stage>] [--export_unassigned]
```
...
`topic_modelling` is the exception — it doesn't write JSON or feed the Excel report. It writes CSVs to `B:\derived` instead; see [Topic Modelling](#topic-modelling).
...
Each sentence is assigned to its highest-scoring topic(s) (ties split evenly); a statement's final score per topic is the proportion of its sentences assigned to that topic. Writes a CSV of per-statement topic proportions to `B:\derived\<output_name>_<YYYYMMDD>.csv` — one column per topic, values between 0 and 1.

Sentence tokenization is cached (`data/cache/` in sample mode, `B:\raw` in restricted mode) and reused on subsequent runs against the same input file.

`--export_unassigned` additionally writes a diagnostic CSV of sentences that matched no topic at all, to `B:\derived\<output_name>_<YYYYMMDD>_unassigned_sentences.csv`. This is a one-time diagnostic, not meant for every run.
